...
const group = (decimal: number, terms: number[], KMap: KMapCell[][], termQueue: number[]): KMapCell[] => {
  ...
  const checkRegion = (w: number, h: number): KMapCell[] | false => {
    ...
    while (Math.abs(r) < Math.abs(h)) {
      const curRow: number = (row + r + rowCount) % rowCount;

      let c: number = 0;

      while (Math.abs(c) < Math.abs(w)) {
        const curCol: number = (col + c + colCount) % colCount;
  ...
  const groupsThatContainCellsGroupedBefore: number[] = [];

  for (let i: number = 0; i < composedGroups.length; i++) {
    const cellsGroupedBefore: KMapCell[] = composedGroups[i].filter((cell: KMapCell) => !termQueue.includes(cell.decimal));

    if (cellsGroupedBefore.length > 0) {
      groupsThatContainCellsGroupedBefore.push(i);
    }
  }

  if (groupsThatContainCellsGroupedBefore.length > 0 && groupsThatContainCellsGroupedBefore.length !== composedGroups.length) {
    return composedGroups.filter((_, i: number) => !groupsThatContainCellsGroupedBefore.includes(i))[0];
  }
